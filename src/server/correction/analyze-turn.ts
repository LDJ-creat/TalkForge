...
export async function analyzeTurnCorrections(
  ...
  const transcript = await resolveTranscript(payload, deps);
  if (!transcript || transcript.turnId !== payload.turnId) {
    throw new JobProcessingError({
      code: "not_found",
      message: `Transcript for turn ${payload.turnId} was not found.`,
      attempts: context.attempts,
      retryable: false,
    });
  }
...
async function buildTranscriptLookup(
  ...
  return new Map(
    turnIds.map((turnId) => [turnId, transcriptsByTurnId.get(turnId) ?? null] as const),
  );
}
